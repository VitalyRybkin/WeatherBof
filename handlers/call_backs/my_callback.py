...
@bot.callback_query_handler(func=lambda call: call.data in [f"{Hourly.table_name}_display",
                                                            f"{Daily.table_name}_display"])
def set_weather_display(call):
    if call.data == f"{Hourly.table_name}_display":
        bot.set_state(call.from_user.id, States.weather_display_hourly, call.message.chat.id)
    ...
    if call.data == f"{Daily.table_name}_display":
        bot.set_state(call.from_user.id, States.weather_display_daily, call.message.chat.id)
